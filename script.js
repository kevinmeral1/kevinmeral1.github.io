...
const texture = textureLoader.load('TexturesCom_Plastic_CarbonFiber_header2.jpg');
...
dodecahedron.rotation.x; // Rotiere den Dodekaeder um 90 Grad um die x-Achse, sodass eine Fläche auf dem Boden liegt

dodecahedron.castShadow = true; // Objekt wirft Schatten
scene.add(dodecahedron);

const light = new THREE.DirectionalLight(0xffffff, 1); // Weiße Richtungslichtquelle
...
window.addEventListener('scroll', function() {
  ...
  if (isElementInViewport(video)) {
    // Spiele das Video ab, wenn es im sichtbaren Bereich ist
    video.play();
  }
});
